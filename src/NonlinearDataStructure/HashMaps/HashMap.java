...
import LinearDataStructure.LinkedList;
...
public class HashMap {
    ...
    public void assign(String key, String value) {
        ...
        if (list.head == null) {
            list.addToHeadMap(key, value);
            return;
        }
        NodeMap current = list.headMap;
        while (current != null) {
            if (current.key == key) {
                current.setKeyValue(key, value);
            }
            if (current.getNextNode() == null) {
                current.setNextNode(new NodeMap(key, value));
                break;
            }
    ...
    public String retrieve(String key) {
        int arrayIndex = this.hash(key);
        NodeMap current = this.hashmap[arrayIndex].headMap;
        while (current != null) {
            if (current.key == key) {
